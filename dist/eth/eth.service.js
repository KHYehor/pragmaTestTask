...
let EthService = class EthService {
    ...
    async saveIntoDBgetGroupById(id, group) {
        ...
        const difference = group.indexes.filter(x => !existingRelations.map(item => item.indexId).includes(Number(x)));
        if (difference.length) {
            const indexes = await this.indexRepository.find({
                indexId: typeorm_2.Raw(alias => `${alias} IN (:...difference)`, { difference: difference.map(el => Number(el)) })
            });
            if (!indexes)
                return;
